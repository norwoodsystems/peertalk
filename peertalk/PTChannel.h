...
- (void)connectToPort:(int)port overUSBHub:(PTUSBHub*)usbHub deviceID:(NSNumber*)deviceID callback:(void(^ _Nullable)(NSError * _Nullable error))callback;
...
- (void)connectToPort:(in_port_t)port IPv4Address:(in_addr_t)address callback:(void(^ _Nullable)(NSError * _Nullable error, PTAddress *address))callback;
...
- (void)listenOnPort:(in_port_t)port IPv4Address:(in_addr_t)address callback:(void(^ _Nullable)(NSError * _Nullable error))callback;
...
- (void)sendFrameOfType:(uint32_t)frameType tag:(uint32_t)tag withPayload:(dispatch_data_t _Nullable)payload callback:(void(^ _Nullable)(NSError * _Nullable error))callback;

// Lower-level method to assign a connected dispatch IO channel to this channel
- (BOOL)startReadingFromConnectedChannel:(dispatch_io_t)channel error:(__autoreleasing NSError** _Nullable)error;
...
- (void)ioFrameChannel:(PTChannel* _Nonnull)channel didReceiveFrameOfType:(uint32_t)type tag:(uint32_t)tag payload:(PTData* _Nullable)payload;

@optional
// Invoked to accept an incoming frame on a channel. Reply NO ignore the
// incoming frame. If not implemented by the delegate, all frames are accepted.
- (BOOL)ioFrameChannel:(PTChannel* _Nonnull)channel shouldAcceptFrameOfType:(uint32_t)type tag:(uint32_t)tag payloadSize:(uint32_t)payloadSize;

// Invoked when the channel closed. If it closed because of an error, *error* is
// a non-nil NSError object.
- (void)ioFrameChannel:(PTChannel* _Nonnull)channel didEndWithError:(NSError* _Nullable)error;

// For listening channels, this method is invoked when a new connection has been
// accepted.
- (void)ioFrameChannel:(PTChannel* _Nonnull)channel didAcceptConnection:(PTChannel* _Nonnull)otherChannel fromAddress:(PTAddress* _Nonnull)address;
